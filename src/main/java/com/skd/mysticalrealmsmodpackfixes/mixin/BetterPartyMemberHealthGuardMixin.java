...
public abstract class BetterPartyMemberHealthGuardMixin {
    ...
    private static float mysticalRealmsModpackFixes$guardMaxHealth(ServerPlayer player, Operation<Float> original) {
        float maxHealth = original.call(player);
        if (!Float.isFinite(maxHealth) || maxHealth <= 0.0F) {
            MRMF_LOGGER.warn("[mystical_realms_modpack_fixes] {} has an invalid max-health attribute ({}); reporting {} to Better Party instead of crashing the server",
    ...
    private static float mysticalRealmsModpackFixes$guardHealth(ServerPlayer player, Operation<Float> original) {
        float health = original.call(player);
        if (!Float.isFinite(health)) {
            MRMF_LOGGER.warn("[mystical_realms_modpack_fixes] {} has a non-finite health value ({}); reporting 0 to Better Party instead of crashing the server",
